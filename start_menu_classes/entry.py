...
class ECreator():                 
    def create_entry(menu): #ISSUE SOMEWHERE, RETAINS CATEGORY AND DESC
        global category
        global desc
        entry_name = input("What is your entry's name ? - ").lower()
        if entry_name in config.entry_dict:
        ...
        ECreator.restart_create_entry(menu)
        menu()
    ...
    def duped_name_denier(name):
        while True:
            name = input("This name already exists within the dictionary. Please choose a new one : - ").lower()
            if name not in config.entry_dict:
                return name
            
    def restart_create_entry(menu):
        global entry_cat_desc 
        entry_cat_desc = {}
        ecreator_recursor = input('Input 1 to create another different entry. - ')
        if ecreator_recursor == "1":
            ECreator.create_entry(menu)